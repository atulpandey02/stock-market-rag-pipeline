...
# ✅ FIX 1: Use env var so it works both locally AND inside Docker/Airflow
# Mac terminal → MINIO_ENDPOINT not set → uses localhost:9000
# Airflow/Docker → MINIO_ENDPOINT=minio:9000 → uses minio:9000
...
def flush_to_minio(minio_client, messages: list) -> bool:
    try:
        ...
        # ✅ FIX 2: Use UTC explicitly — matches Airflow {{ ds }} which is also UTC
        # This ensures consumer file paths always match what Airflow expects
        ...
        logger.info(
            f"Wrote {len(messages)} records → s3://{MINIO_BUCKET}/{object_name} "
...
def main():
    ...
    logger.info(f"Kafka Server   : {KAFKA_BOOTSTRAP_SERVER}")
    logger.info(f"Topic          : {KAFKA_TOPIC_REALTIME}")
