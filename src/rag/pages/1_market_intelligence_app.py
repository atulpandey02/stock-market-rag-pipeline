...
Finnhub News → Pinecone → Groq Llama3

Uses st.chat_input for reliable submission (no form/button state issues)
Uses llama-3.1-8b-instant (latest fast Groq model)
"""
...
# ── Path setup — works both standalone and as multipage ───────────────────────
_this_dir = os.path.dirname(os.path.abspath(__file__))
_root_dir  = os.path.dirname(_this_dir)   # src/rag/
...
.ticker-bar {
    ...
    font-size: 12px; margin-bottom: 20px; flex-wrap: wrap;
}
    ...
    for i, src in enumerate(sources, 1):
        ...
        st.markdown(f"""
            <div class='source-card'>
                <div class='source-title' style='color:#e2e8f0;font-weight:500;margin-bottom:3px;'>
                    [{i}] {src.get('title', 'N/A')}
                    {badge}
                    <span class='score-pill'>{score:.3f}</span>
                </div>
                <div class='source-meta' style='color:#64748b;font-size:11px;'>
...
with st.sidebar:
    ...
    selected_stock = st.selectbox("Stock filter", stocks,
    ...
    st.markdown("""
        <div style='font-size:11px;color:#64748b;line-height:1.8;'>
            ...
            <div>🧠 <b style='color:#e2e8f0;'>LLM</b> Groq Llama3.1-8b</div>
        </div>
    """, unsafe_allow_html=True)
...
# ── Header ────────────────────────────────────────────────────────────────────
st.markdown("<h1 class='header-title'>📈 Market Intelligence</h1>",
            unsafe_allow_html=True)
st.markdown("""
    <p class='header-sub'>
        RAG-powered financial analysis · Finnhub News · Pinecone · Groq Llama3.1
    </p>
""", unsafe_allow_html=True)
...
# ── Input — st.chat_input (no form/button state issues) ──────────────────────
