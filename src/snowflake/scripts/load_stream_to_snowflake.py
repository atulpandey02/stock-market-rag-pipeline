...
def create_snowflake_table(conn):
    ...
    ✅ FIX 3: Column names match spark_stream_batch_processor.py output:
      ma_3m, ma_5m (NOT ma_15m, ma_1h — we changed window sizes for portfolio)
      volatility_3m, volatility_5m
      volume_sum_3m, volume_sum_5m
      window_3m_end, window_5m_end (NOT window_15m_end, window_1h_end)
    """
    ddl = f"""
    CREATE TABLE IF NOT EXISTS
        {SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.{SNOWFLAKE_TABLE} (
            ...
            window_3m_end   TIMESTAMP,
            window_5m_end   TIMESTAMP,
            ma_3m           FLOAT,
            ma_5m           FLOAT,
            volatility_3m   FLOAT,
            volatility_5m   FLOAT,
            volume_sum_3m   BIGINT,
            volume_sum_5m   BIGINT,
            batch_id        STRING,
...
def load_to_snowflake(conn, df: pd.DataFrame):
    ...
    ✅ FIX 6: Column names match our updated Spark output
              (ma_3m/ma_5m not ma_15m/ma_1h)
    """
    logger.info(MINI_SEP)
    ...
    try:
        ...
        # ✅ Only insert columns that exist in Snowflake table
        snowflake_cols = [
            'symbol', 'window_start', 'window_3m_end', 'window_5m_end',
            'ma_3m', 'ma_5m', 'volatility_3m', 'volatility_5m',
            'volume_sum_3m', 'volume_sum_5m', 'batch_id'
        ]
